...
class PlayState extends MusicBeatState
{
	...
	var sky:BGSprite;
	var skyCorrupt:BGSprite;
	var NEO:BGSprite;
	var fire:BGSprite;
	var Boppers2:BGSprite;
	var Boppers3:BGSprite;
	var stagecorrupted:BGSprite;
	var corruptedBoppers:BGSprite;
	...
	var grpLimoDancers_D:FlxTypedGroup<BackgroundDancer_D>;
	var grpLimoDancers_D_dark:FlxTypedGroup<BackgroundDancer_D_dark>;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'neo-bopeebo' | 'neo-fresh' | 'neo-dadbattle':
					curStage = 'stageneo';
			    case 'neo-spookeez' | 'neo-south':
				    curStage = 'spookyneo';
		        case 'illusion':
					curStage = 'monsterneo';
			    case 'neo-pico' | 'neo-blammed' | 'neo-philly':
				    curStage = 'phillyneo';
			    case 'neo-satin-panties' | 'neo-high' | 'neo-milf':
				    curStage = 'limoneo';
				case 'neo-cocoa' | 'neo-eggnog':
					curStage = 'mallneo';
				case 'hallucination':
				...
				case 'pico_d', 'philly-nice_d':
					curStage = 'philly-D';
				case 'blammed_d':
					curStage = 'philly-D_dark';
				case 'satin-panties_d' | 'high_d':
					curStage = 'limo-D';
				case 'awooga' | 'milf-d':
					curStage = 'limo-D_dark';
				default:
					curStage = 'stage';
			}
		}
		...
		switch (curStage)
		{
			...
			case 'stageneo': //NEO
			    var bg:BGSprite = new BGSprite('NEO/backwall', -500, -200, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('NEO/stage', -650, 600, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				stageFront.scrollFactor.set(0.9, 0.9);
				add(stageFront);

				stageLights = new BGSprite('NEO/front_lights', -550, 450, 0.9, 0.9);
				stageLights.setGraphicSize(Std.int(stageFront.width * 1.0));
				...
				if(!ClientPrefs.lowQuality) {
					stageCurtains = new BGSprite('NEO/lights', -500, -300, 0.9, 0.9);
					stageCurtains.setGraphicSize(Std.int(stageCurtains.width * 0.9));
					stageCurtains.updateHitbox();
			...
			case 'spookyneo': //NEO
			    var bg:BGSprite = new BGSprite('NEO/buildings', -300, -100, 0.8, 0.8);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.8, 0.8);
				add(bg);

				var signs:BGSprite = new BGSprite('NEO/neonsigns', -300, -35, 0.8, 0.8);
				signs.antialiasing = true;
				signs.scrollFactor.set(0.8, 0.8);
				add(signs);

				speakers = new BGSprite('NEO/SpookySpeakersB', -550, -70, 0.9, 0.9, ['BG SPEAKERS'], true);
				speakers.setGraphicSize(Std.int(speakers.width * 1.0));
				speakers.updateHitbox();
				speakers.antialiasing = true;
				add(speakers);

				speakers2 = new BGSprite('NEO/SpookySpeakersP', 1300, -70, 0.9, 0.9, ['BG SPEAKERS PINK'], true);
				speakers2.setGraphicSize(Std.int(speakers2.width * 1.0));
				speakers2.updateHitbox();
				speakers2.antialiasing = true;
				add(speakers2);
			case 'monsterneo': //NEO
			    var bg:BGSprite = new BGSprite('NEO/buildingsevil', -300, -300, 1, 1);
				bg.antialiasing = true;
				bg.scrollFactor.set(0.8, 0.8);
				add(bg);

				var signs:BGSprite = new BGSprite('NEO/floorevil', -300, -35, 1, 1);
				signs.antialiasing = true;
				signs.scrollFactor.set(0.8, 0.8);
				add(signs);

				mouth = new BGSprite('NEO/MOUTH', 350, 770, 1, 1, ['MOUTHANIM'], true);
				mouth.antialiasing = true;
				mouth.scrollFactor.set(0.9, 0.9);
				mouth.setGraphicSize(Std.int(mouth.width * 1.0));
				add(mouth);

				tvL = new BGSprite('NEO/XO_TV_L', -250, 150, 1, 1, ['BG SPEAKERS TVS'], true);
				...
				tvR = new BGSprite('NEO/XO_TV_R', 1000, 120, 1, 1, ['BG SPEAKERS TVS OtherSide'], true);
				...
				fgFog = new BGSprite('NEO/fog', -300, -300, 1, 1);
				fgFog.active = false;
		...
		case 'phillyneo': //Week 3
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('NEO/sky', -100, 0, 0.1, 0.1);
				    bg.scrollFactor.set(0.1, 0.1);
					add(bg);
				}

				var city:BGSprite = new BGSprite('NEO/phillybuildings', -10, 0, 0.3, 0.3);
				...
				for (i in 0...2)
				{
					var light:BGSprite = new BGSprite('NEO/light' + i, city.x, city.y, 0.3, 0.3);
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('NEO/roads', -40, 50);
					add(streetBehind);
				}

				phillyTrain = new BGSprite('NEO/train', 2000, 360);
				add(phillyTrain);

				trainSound = new FlxSound().loadEmbedded(Paths.sound('car_passes'));
				trainSound.volume = 0.6;
				CoolUtil.precacheSound('car_passes');
				FlxG.sound.list.add(trainSound);

				var street:BGSprite = new BGSprite('NEO/alleyway', -40);
				add(street);
				
			case 'limoneo': //Week 4
			    skyBG = new BGSprite('NEO/limoSunset', -780,-400);
				skyBG.scrollFactor.set(0, 0);
				skyBG.antialiasing = true;
				skyBG.setGraphicSize(Std.int(skyBG.width*1.65));
				add(skyBG);

				limo = new BGSprite('NEO/limoDrive', 150, 375, 1, 1, ['Limo stage'], true);
				limo.setGraphicSize(2950, 750);
				limo.updateHitbox();

				cloud = new BGSprite('NEO/cloud', -950, 75);
				cloud.velocity.x = 20;
				cloud.scrollFactor.set(0.65, 0.65);
				cloud.antialiasing = true;
				cloud.setGraphicSize(Std.int(cloud.width*3.0));

		case 'mallneo': //Week 5 - Cocoa, Eggnog
		        sky = new BGSprite('NEO/newyears/sky', -1000, -500);
				...
				skyCorrupt = new BGSprite('NEO/newyears/sky2', -1000, -500);
				...
				NEO = new BGSprite('NEO/newyears/NEO_Fireworks', 300, -100, 1, 1, ['Week 5 Firework Blue'], true);
				...
				fire = new BGSprite('NEO/newyears/FireworkNeo', 300, -100, 1, 1, ['Week 5 Firework'], true);
				...
				bgEscalator = new BGSprite('NEO/newyears/buildings', -1100, -600);
				...
				fgSnow = new BGSprite('NEO/newyears/mainstage', -450, -550);
				fgSnow.active = false;
				fgSnow.antialiasing = true;
				fgSnow.setGraphicSize(Std.int(fgSnow.width * 1.25));
				add(fgSnow);

				stagecorrupted = new BGSprite('NEO/newyears/mainstagecorrupted', -450, -550);
				...
				Boppers2 = new BGSprite('NEO/newyears/CrowdRight', 350, 150, 1, 1, ['CROWD RIGHT'], true);
				Boppers2.antialiasing = true;
				Boppers2.updateHitbox();

				Boppers3 = new BGSprite('NEO/newyears/CrowdLeft', -800, 85, 1, 1, ['CROWD LEFT'], true);
				Boppers3.antialiasing = true;
				Boppers3.updateHitbox();

		case 'mallEvilneo':
			    skyCorrupt = new BGSprite('NEO/newyears/skycorrupted', -1000, -500);
				skyCorrupt.antialiasing = true;
				skyCorrupt.scrollFactor.set(0.2, 0.2);
				skyCorrupt.active = false;
				skyCorrupt.setGraphicSize(Std.int(skyCorrupt.width * 0.8));
				...
				bgEscalator = new BGSprite('NEO/newyears/Scary-buildings', -1100, -600);
				bgEscalator.antialiasing = true;
				...
				fgSnow = new BGSprite('NEO/newyears/mainstagecorruption', -450, -550);
				fgSnow.active = false;
				fgSnow.antialiasing = true;
				fgSnow.setGraphicSize(Std.int(fgSnow.width * 1.25));
				add(fgSnow);

				corruptedBoppers = new BGSprite('NEO/newyears/Corrupted_bg_characters', -5, -25, 1, 1, ['neo garcello instance 1'], true);
				corruptedBoppers.antialiasing = true;
				corruptedBoppers.updateHitbox();
				add(corruptedBoppers);

				var fgTables:BGSprite = new BGSprite('NEO/newyears/maintablescorruption', -450, -550);
				fgTables.active = false;
				fgTables.antialiasing = true;
				fgTables.setGraphicSize(Std.int(fgTables.width * 1.25));
				add(fgTables);

				fgFog = new BGSprite('NEO/fog', -450, -550, 1, 1);
				fgFog.active = false;
				fgFog.antialiasing = true;
				fgFog.setGraphicSize(Std.int(fgFog.width * 1.30));
				FlxTween.tween(fgFog,{x: fgFog.x + 100}, 5.5,{ease:FlxEase.cubeOut,type:PINGPONG});
			...
			case 'philly-D': //Week 3
				if(!ClientPrefs.lowQuality) {
					var bg:BGSprite = new BGSprite('d-side/sky', -100, 0, 0.1, 0.1);
					add(bg);
				}
				...
				var city:BGSprite = new BGSprite('d-side/city', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);
				...
				if(!ClientPrefs.lowQuality) {
					var streetBehind:BGSprite = new BGSprite('d-side/behindTrain', -40, 50);
					add(streetBehind);
				}

				phillyTrain = new BGSprite('d-side/train', 2000, 360);
				add(phillyTrain);
				...
				var street:BGSprite = new BGSprite('d-side/street', -40, 50);
				add(street);
				
				phillyCrowd = new BGSprite('d-side/Crowd1', 120, 360, 1, 1, ['Crowd1'], true);
				phillyCrowd.antialiasing = true;
				phillyCrowd.setGraphicSize(Std.int(phillyCrowd.width * 0.85));
				phillyCrowd.updateHitbox();
				add(phillyCrowd);

			case 'philly-D_dark': //Week 3
				var bg:BGSprite = new BGSprite('d-side/skyDark', -100, 0, 0.1, 0.1);
				add(bg);

				var city:BGSprite = new BGSprite('d-side/cityDark', -10, 0, 0.3, 0.3);
				city.setGraphicSize(Std.int(city.width * 0.85));
				city.updateHitbox();
				add(city);
				...
				var streetBehind:BGSprite = new BGSprite('d-side/behindTrainDark', -40, 50);
				add(streetBehind);

				phillyTrain = new BGSprite('d-side/trainDark', 2000, 360);
				add(phillyTrain);
				...
				var street:BGSprite = new BGSprite('d-side/streetDark', -40, 50);
				add(street);
				
				phillyCrowd = new BGSprite('d-side/Crowd2DARK', -20, 220, 1, 1, ['Crowd2'], true);
				phillyCrowd.antialiasing = true;
				phillyCrowd.setGraphicSize(Std.int(phillyCrowd.width * 0.85));
				phillyCrowd.updateHitbox();
				add(phillyCrowd);
			...
			case 'limo-D': //Week 4
				var skyBG:BGSprite = new BGSprite('d-side/limoSunset', -320, -250, 0.1, 0.1);
				add(skyBG);

				if(!ClientPrefs.lowQuality) {
					bgLimo = new BGSprite('d-side/bgLimo', -150, 480, 0.4, 0.4, ['background limo pink'], true);
					add(bgLimo);

					grpLimoDancers_D = new FlxTypedGroup<BackgroundDancer_D>();
					add(grpLimoDancers_D);

					for (i in 0...5)
					{
						var dancer:BackgroundDancer_D = new BackgroundDancer_D((370 * i) + 130, bgLimo.y - 400);
						dancer.scrollFactor.set(0.4, 0.4);
						grpLimoDancers_D.add(dancer);
					}
				}

				limo = new BGSprite('d-side/limoDrive', -120, 550, 1, 1, ['Limo stage'], true);

				fastCar = new BGSprite('d-side/fastCarLol', -300, 160);
				fastCar.active = true;

			case 'limo-D_dark': //Week 4
				var skyBG:BGSprite = new BGSprite('d-side/limoSunsetNight', -320, -250, 0.1, 0.1);
				add(skyBG);

				if(!ClientPrefs.lowQuality) {
					bgLimo = new BGSprite('d-side/bgLimoDARK', -150, 480, 0.4, 0.4, ['background limo pink'], true);
					add(bgLimo);

					grpLimoDancers_D_dark = new FlxTypedGroup<BackgroundDancer_D_dark>();
					add(grpLimoDancers_D_dark);

					for (i in 0...5)
					{
						var dancer:BackgroundDancer_D_dark = new BackgroundDancer_D_dark((370 * i) + 130, bgLimo.y - 400);
						dancer.scrollFactor.set(0.4, 0.4);
						grpLimoDancers_D_dark.add(dancer);
					}
				}

				limo = new BGSprite('d-side/limoDriveDARK', -120, 550, 1, 1, ['Limo stage'], true);

				fastCar = new BGSprite('d-side/fastCarLolDARK', -300, 160);
				fastCar.active = true;
		}
		...
		if (curStage.startsWith('monsterneo'))
			add(fgFog);
		...
		if(curStage == 'phillyneo') {
			phillyCityLightsEvent = new FlxTypedGroup<BGSprite>();
			for (i in 0...2)
			{
				var light:BGSprite = new BGSprite('NEO/light' + i, -10, 0, 0.3, 0.3);
				light.visible = false;
				light.setGraphicSize(Std.int(light.width * 0.85));
				light.updateHitbox();
				phillyCityLightsEvent.add(light);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				switch(curStage)
				{
					case 'stageneo' | 'spookyneo' | 'phillyneo' | 'limoneo':
					...
					case 'mallneo':
						...
						antialias = false;
		                add(boyfriend);	
					case 'mallEvilneo' | 'monsterneo': 
						introAlts = introAssets.get('neo');
						antialias = false;
						gf.visible = false;
					case 'stage-D' | 'stage-D_dark' | 'spooky-D' | 'spooky-D_monster' | 'philly-D' | 'philly-D_dark' | 'limo-D' | 'limo-D_dark':	
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'limo-D':
				if(!ClientPrefs.lowQuality) {
					grpLimoDancers_D.forEach(function(dancer:BackgroundDancer_D)
					{
						dancer.dance();
					});
				}
			...
			case 'limo-D_dark':
				if(!ClientPrefs.lowQuality) {
					grpLimoDancers_D_dark.forEach(function(dancer:BackgroundDancer_D_dark)
					{
						dancer.dance();
					});
				}
