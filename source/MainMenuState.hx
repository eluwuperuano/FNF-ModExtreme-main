...
class MainMenuState extends MusicBeatState
{
	...
	public static var modextremeVersion:String = '0.1.1'; //This is also used for Discord RPC
	...
	override function create()
	{
		...
		var yScroll:Float = Math.max(0.25 - (0.05 * (optionShit.length - 4)), 0.1);
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menuBGUmbral'));
		bg.scrollFactor.set(0, yScroll);
		bg.setGraphicSize(Std.int(bg.width * 1.175*scaleRatio ));
		bg.updateHitbox();
		bg.screenCenter();
		...
		umbral = new FlxSprite(-80).loadGraphic(Paths.image('menuDesatUmbral'));
		umbral.scrollFactor.set(0, yScroll);
		umbral.setGraphicSize(Std.int(umbral.width * 1.175*scaleRatio ));
		umbral.updateHitbox();
		umbral.screenCenter();
		umbral.visible = false;
		umbral.antialiasing = ClientPrefs.globalAntialiasing;
		umbral.color = 0xFFfd719b;
		add(umbral);
		...
		/*if(optionShit.length > 6) {
			...
			var menuItem:FlxSprite = new FlxSprite(0, (0 * 140)  + offset);
			...
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(0, (2 * 140)  + offset);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
			...
			var offset:Float = 108 - (Math.max(optionShit.length, 4) - 4) * 80;
			var menuItem:FlxSprite = new FlxSprite(0, (3 * 140)  + offset);
			menuItem.scale.x = scale;
			menuItem.scale.y = scale;
		...
		var versionShit:FlxText = new FlxText(12, ClientPrefs.getResolution()[1] - 66, 0, "Mod Extreme v" + modextremeVersion, 12);
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				else
				{
					...
					if(ClientPrefs.flashing) FlxFlicker.flicker(umbral, 1.1, 0.15, false);
